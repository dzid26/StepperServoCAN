...
volatile bool enableTorque = false;  	// feedforward and close loop control
...
bool StepperCtrl_processMotion(void) {
	...
	int32_t currentLoc = StepperCtrl_updateCurrentLocation(); //CurrentLocation

	loopError = desiredLocation - currentLoc;
	int32_t speed_raw = (currentLoc - lastLoc) * (int32_t) SAMPLING_HZ; // rev/s/65536
	...
	if (base_speed_test_mode) {
		...
		if (enableSoftOff) {
			...
			closeLoop = 0;
			iTerm = control;
		} else if (enableTorque) {
			int16_t closeLoopSat = 0;
			if (enableCloseLoop) { // todo move close loop out of fast motion task
				int32_t errorSat;
				int16_t pTerm;
				int16_t dTerm;
				...
				if (enableRelative) {
					...
					error = desiredLoc_slow - currentLoc;
				}
				...
				// PID - (I)ntegral term
				iTerm_accu += errorSat;
				iTerm = (int16_t)(iTerm_accu  * pPID.Ki / (int16_t)SAMPLING_PERIOD_uS / CTRL_PID_SCALING); //it's safe to cast to int16_t as iTerm_accu / CTRL_PID_SCALING cannot be much bigger than maxEachTerm since last time because iTerm_accu uses limited errorSat when acumulating error
				...
				closeLoopSat = clip(closeLoop, -closeLoopMax, closeLoopMax);
				iTerm = iTerm - clip(closeLoop - closeLoopSat, -iTerm, iTerm);
			} else {
				closeLoop = 0;
				lastError = 0;
			}
			// add feedforward and close loop (if enabled)
			...
			if (enableCloseLoop){ // todo probably move this in front of the enableCloseLoop conditional
				// this will initialize integral part in case of switching form torque control to close loop
				iTerm = iTerm - clip(controlSum - control, -iTerm, iTerm);
			} else {
				iTerm = control;
			}
		} else {
		...
		// if (!enableCloseLoop) {
			// todo: fix - this is calculated always based on low res iTerm which defeats the purpose of high resolution iTerm_accu
