...
const float motor_rated_hold_torque =  (float) 116;  // Ncm - (values in datasheets represents both phases energized with rated current AND are often inflated for small motors)
...
volatile int16_t phase_R = 3600;         // mOhm -      it's best to measure this with multimeter
